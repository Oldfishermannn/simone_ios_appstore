...
struct ElectronicSignatureView: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            // Static pedalboard layer (大图专属)——5 款 iconic guitar pedals 按真
            // 实比例排在 synth 下方。绘制量大 (~400 ops) 但只 Metal-rasterize 一次，
            // SwiftUI `.opacity(deckAlpha)` 做 compositor-level 淡入，零 per-frame 成本。
            Canvas { context, size in
                drawPedalboard(ctx: context, size: size)
            }
            .drawingGroup()
            .opacity(deckAlpha)
            .allowsHitTesting(false)

            // Dynamic overlay — CRT waveform + POWER LED + panel lamp wash +
            // keyboard underglow (最后两者仍 gated by zc > 0.05，大图 synth 本体细节)。
    ...
    // MARK: - 大图专属: Pedalboard v4 (5 款 iconic guitar pedals，synth 下方一字排开)
    //
    // CEO v4 反馈："把灯 光和耳机去掉 画出这些效果器 整齐排列在合成器下方/上方
    //              要求 比例和颜色必须一致"
    //
    // 换方向：从"房间环境物"切到"琴手另一副装备"——键盘 + 吉他效果器一字排开
    // 是工作室真实场景（很多 synth 玩家把 pedals 接进 signal chain）。每一款都是
    // 玩家一眼能认出的 iconic unit，按真实长宽比 + 真实色落位：
    //   · TS808 Tube Screamer (Ibanez)  — jade 绿，纵长（W:H ≈ 0.58）
    //   · BigSky (Strymon)              — 天蓝，横宽（W:H ≈ 1.42）
    //   · TimeLine (Strymon)            — 暗灰，横宽（W:H ≈ 1.42）
    //   · RAT (ProCo)                   — 哑光黑，纵长（W:H ≈ 0.72）
    //   · Centavo (Klon 风)             — 金黄，横宽（W:H ≈ 1.32）
    //
    // 5 原则对齐：
    //   · 物件感 > UI — 每台都是 3D 金属盒 (chassis bevel + screws + knobs)，不是 UI chip
    //   · 沉浸优先 — 全静态，不晃不闪（pedal LED 常亮不 blink）
    //   · 克制温度 — 绿/蓝/灰/黑/金 真实配色，不全调暖，不渲染氛围
    //   · 侧光 — chassis 统一 upper-left highlight + lower-right shadow，同一光向
    //   · 动而非跳 — synth 本体保留音频联动，pedalboard 不参与
    private func drawPedalboard(ctx: GraphicsContext, size: CGSize) {
        let L = layout(for: size)
        let body = L.body
    ...
    private func drawPedalTS808(ctx: GraphicsContext, rect: CGRect) {
        ...
        // 3 knobs: DRIVE (top-left), TONE (top-right), LEVEL (bottom-center) — classic TS layout
        ...
        // Large footswitch bottom center
        drawPedalFootswitch(
            ctx: ctx,
    ...
    // Shared Strymon chassis: dot-matrix display top-left + big encoder w/ LED ring
    // top-right + 5 small knobs middle row + 3 footswitches at bottom
    private func drawStrymonChassis(
        ...
        // Display glyphs (amber/cyan segments)
        for i in 0..<3 {
        ...
        // Big encoder upper-right with LED dot ring
    ...
    private func drawPedalRAT(ctx: GraphicsContext, rect: CGRect) {
        ...
        let w = rect.width
        let h = rect.height
        // 3 knobs top row: DISTORTION / FILTER / VOLUME
        ...
        // Small white label boxes under each knob (DIST / FILT / VOL abbreviated)
        ...
        // Red LED between knobs and logo
        drawPedalLED(
            ctx: ctx,
            center: CGPoint(x: rect.midX, y: rect.minY + h * 0.46),
    ...
    private func drawPedalCentavo(ctx: GraphicsContext, rect: CGRect) {
        ...
        let w = rect.width
        let h = rect.height
        // 3 knobs top row: GAIN / TREBLE / OUTPUT (left side of pedal, leaving room for horse silhouette)
        ...
        // Centaur silhouette on the right (dark ink on gold)
    ...
    // Klon centaur silhouette — stylized, dark ink stamp look on gold enclosure
